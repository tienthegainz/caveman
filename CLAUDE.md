...
- `buildRuleset(mode)` — builds the activation ruleset string injected at session start. Reads SKILL.md (single source of truth) and filters the intensity table + examples to the active level; falls back to a hardcoded minimum-viable ruleset when SKILL.md is absent (standalone hook installs). Honors a `CAVEMAN_SKILL_PATH` env override before the plugin-relative path — the Copilot CLI install copies SKILL.md next to its hooks and points here so it injects the full ruleset, not the fallback. Shared by the Claude `caveman-activate.js` hook and the Copilot `caveman-copilot-activate.js` hook. Returns the short "behavior defined by /caveman-<mode> skill" line for independent modes (`commit`/`review`/`compress`, exported as `INDEPENDENT_MODES`).

### `src/hooks/caveman-copilot-activate.js` / `caveman-copilot-mode-tracker.js` — Copilot CLI hooks

GitHub Copilot CLI has its own hook system (`~/.copilot/hooks/*.json`, overridable via `COPILOT_HOME`) with a different contract from Claude Code:
- **`caveman-copilot-activate.js`** — `sessionStart` hook. Resolves the mode (an explicit `off` from env/config always wins; otherwise a persisted flag overrides the configured default), writes the flag at `$COPILOT_HOME/.caveman-active`, and injects the ruleset by writing **single-line JSON `{ "additionalContext": "..." }`** to stdout. Emits `{}` (no-op) in `off` mode or on any error — must never block the CLI.
- **`caveman-copilot-mode-tracker.js`** — `userPromptSubmitted` hook. Copilot CLI does **not** process this event's output (can't block or inject), so it is side-effect only: it persists mode picks to the flag for the next session. It extracts the requested mode from two prompt shapes: a literal `/caveman <mode>`, **and** the skill-invocation rewrite Copilot produces once a skill named `caveman` is installed (`... invoke the "caveman" skill ... help with: <mode>`). Only valid intensity levels switch the flag (a `help with: explain X` invocation leaves it untouched). Also handles natural-language activate/deactivate. Always emits `{}`.

**Flag location.** Both Copilot hooks resolve the flag via `CAVEMAN_FLAG_DIR` (env, resolved against the hook's `cwd`) → else `$COPILOT_HOME/.caveman-active`. The installer sets `CAVEMAN_FLAG_DIR` only for **repo-scoped** installs (`.github/hooks/caveman`, repo-relative because `cwd: "."` = repo root), so a repo install keeps its mode flag repo-local (gitignored via a generated `.github/hooks/caveman/.gitignore`) instead of polluting the global `~/.copilot/.caveman-active`. User-scoped installs leave `CAVEMAN_FLAG_DIR` unset → global flag.

Both reuse `caveman-config.js` helpers (`getDefaultMode`, `safeWriteFlag`, `readFlag`, `buildRuleset`, `INDEPENDENT_MODES`). Installed by `installCopilotCli()` in `bin/install.js`, which copies the two scripts + `caveman-config.js` + a `caveman-skill.md` copy of SKILL.md into `~/.copilot/hooks/` and generates `caveman.json` wiring both hooks (with `CAVEMAN_SKILL_PATH` set so the full ruleset is injected). There is no statusline (Copilot CLI exposes no plugin-writable badge) and no per-turn reinforcement (the `userPromptSubmitted` output limitation). Tests in `tests/test_copilot_hooks.js`.

**Two install scopes.** `--only copilot-cli` installs **user-level** hooks into `~/.copilot/hooks/` (absolute `node` path + absolute script paths, machine-local) plus the caveman **Agent Skills** into `~/.copilot/skills/`. Adding `--repo` installs **repo-scoped** hooks into `<repo>/.github/hooks/caveman.json` + self-contained scripts under `<repo>/.github/hooks/caveman/` and skills under `<repo>/.github/skills/` — these are **committable**, so the hook config uses bare `node` (must be on PATH) + repo-relative paths + `cwd: "."`. The target repo is found by `findTargetRepoRoot()` (walks up from cwd for `.git`). `buildRuleset` resolves the co-located `caveman-skill.md` via `__dirname`, so the full ruleset injects regardless of scope/cwd. Uninstall the repo copy with `--uninstall --repo` from inside that repo.

**Skills vs. hooks vs. slash commands.** `installCopilotSkills()` copies `COPILOT_SKILLS` (caveman, caveman-commit, caveman-review, caveman-compress, caveman-help) from `skills/<name>/` into the scope's skills dir. Per GitHub's customization matrix, Copilot CLI supports **Agent Skills** (auto-loaded when relevant) and **hooks** but NOT **prompt files**, so there is no literal `/caveman` slash command in the CLI — caveman is delivered via the always-on `sessionStart` hook + auto-loaded skills + the prompt-hook mode switcher. Skills install requires a local clone (multi-file folders aren't downloaded piecemeal); hooks fall back to download.
...
| Copilot CLI | User-level hooks in `~/.copilot/hooks/` (overridable via `COPILOT_HOME`): `caveman.json` config wires a `sessionStart` hook (`caveman-copilot-activate.js`) that injects the ruleset via `{ "additionalContext": ... }`, and a `userPromptSubmitted` hook (`caveman-copilot-mode-tracker.js`) that persists mode picks. Also installs the caveman Agent Skills into `~/.copilot/skills/` — so `/caveman` invokes the caveman skill, and the prompt hook reads Copilot's skill-invocation rewrite to persist the chosen mode. Installed by `installCopilotCli()` in `bin/install.js` (provider id `copilot-cli`). Flag at `$COPILOT_HOME/.caveman-active` (user scope) or repo-local via `CAVEMAN_FLAG_DIR` (repo scope). | Yes — `sessionStart` hook injects rules every session |
...
| Copilot (editor extension) | `npx skills add ... -a github-copilot` (soft probe — pass `--only copilot`); per-repo `.github/copilot-instructions.md` + `AGENTS.md` via `--with-init`. The VS Code / Cursor extension has no hook system — for the **Copilot CLI** with always-on hooks use the `copilot-cli` provider (row above). | Yes — repo-wide instructions |
